 ...
 * `captured` and invokes `onCapture` exactly once.
 */
export function useHoldTimer(
  durationMs: number,
  onCapture: () => void,
  onTick?: (progress: number, dtMs: number) => void,
): UseHoldTimerResult {
  ...
  // Keep the latest callbacks without making the RAF loop depend on them.
